...
# Captures every frame deterministically from the reel's JS clock (no lag, full quality).
...
ffmpeg -y -framerate 30 -i "$FRAMES/f-%04d.png" \
  -c:v libx264 -pix_fmt yuv420p -crf 15 -preset slow -movflags +faststart \
  "$OUT" >/dev/null 2>&1
rm -rf "$FRAMES"

echo "Done → nature-of-sound-reel.mp4  (silent; add audio in your editor or ask Claude)"
open "$(cd ../.. && pwd)"
